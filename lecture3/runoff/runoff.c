...
bool print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].votes > voter_count / 2 and not candidates[i].eliminated)
        {
            printf("%s\n", candidates[i].name);
            return true;
        }
...
int find_min(void)
{
    ...
    for (int candidate = 0; candidate < candidate_count; candidate++)
    {
        if (candidates[candidate].votes < min_votes and not candidates[candidate].eliminated)
        {
